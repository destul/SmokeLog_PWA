...
function App() {
  ...
  const [productActionsProduct, setProductActionsProduct] = useState<Product | null>(null)
  ...
  async function hideProduct(product: Product) {
    ...
    setProductActionsProduct(null)
    setNotice(`Приховано: ${product.name}`)
    await refresh()
  }

  async function showProduct(product: Product) {
    await saveProduct({ ...product, active: true, updatedAt: new Date().toISOString() })
    setProductActionsProduct(null)
    setNotice('Продукт показано.')
    await refresh()
  }
